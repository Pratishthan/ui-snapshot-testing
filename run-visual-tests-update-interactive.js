...
function findMostRecentScreenshot(storyId) {
  ...
  const matchingFiles = files
    .filter(
      (file) => file.startsWith(`${sanitizedId}-`) && file.endsWith(".png"),
    )
    .map((file) => {
...
async function copySnapshotsToReference(storyIds, failures) {
  ...
  for (const storyId of storyIds) {
    ...
    const referencePositionPath = path.join(
      snapshotDir,
      `${sanitizedId}.positions.json`,
    );
    ...
    try {
      ...
      const positionPathAlongside = path.join(
        screenshotDir,
        `${screenshotBaseName}.positions.json`,
      );
            ...
            const findPositionSnapshot = (dir) => {
              ...
              for (const entry of entries) {
                ...
                if (entry.isDirectory()) {
                  ...
                  entry.name === `${sanitizedId}.positions.json` ||
                  ...
                  if (
                    baseName.startsWith(sanitizedId) ||
                    baseName === sanitizedId
                  ) {
                    return fullPath;
                  }
                }
          ...
          if (existsSync(screenshotsDir)) {
            ...
            const matchingPositionFiles = files
              .filter((file) => {
                ...
                return (
                  file.endsWith(".positions.json") &&
                  (fileBase.startsWith(sanitizedId) ||
                    screenshotBase.startsWith(sanitizedId))
                );
              })
              .map((file) => {
                const filePath = path.join(screenshotsDir, file);
      ...
      if (actualPositionPath && existsSync(actualPositionPath)) {
        ...
        console.log(
          `   ✓ Updated image snapshot for ${storyId} (position snapshot not found)`,
        );
      }
    } catch (error) {
      results.errors.push({
...
async function main() {
  ...
  if (!failures || failures.length === 0) {
    console.error(
      "❌ No failures found in logs/visual-test-failures.jsonl or logs/visual-test-results.json",
    );
    console.error("");
    console.error("💡 To generate failure data, run:");
    console.error("   npx ui-snapshot-testing run");
    console.error("");
    console.error("💡 Or view existing failures in the HTML report:");
    console.error("   npx ui-snapshot-testing report");
    process.exit(1);
  }
  ...
  const prompt = new MultiSelect({
    name: "stories",
    message:
      "Select stories to update snapshots (use space to select, enter to confirm)",
    choices: choices,
  ...
  selectedStoryIds.forEach((storyId, index) => {
    ...
    const pathInfo = failure?.importPath
      ? ` (${truncate(failure.importPath, 50)})`
      : "";
  ...
  console.log(
    `   This will copy the actual screenshots from the most recent test run to the reference directory.\n`,
  );
  ...
  try {
    ...
    console.log(
      `   Updated: ${results.updated.filter((r) => r.type === "image").length} image snapshot(s)`,
    );
    if (results.updated.some((r) => r.type === "position")) {
      console.log(
        `   Updated: ${results.updated.filter((r) => r.type === "position").length} position snapshot(s)`,
      );
    }
    if (results.skipped.length > 0) {
      console.log(
        `   Skipped: ${results.skipped.length} story/stories (no actual screenshot found)`,
      );
    }
