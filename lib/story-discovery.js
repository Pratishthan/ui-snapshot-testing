...
export const fetchStoriesFromStorybook = async (
  ...
  let filtered = entries
    ...
    .filter(
      (entry) =>
        !matchesExclusionPatterns(entry, config.snapshot.filters.exclusions),
    );

  // Determine matchers (cascading: specific > global)
  const globalMatcher = config.snapshot.testMatcher;
  ...
  filtered = filtered
    ...
    .filter((entry) =>
      matchesPathFilters(entry, config.snapshot.filters.includePaths),
    )
    .filter((entry) =>
      matchesStoryIdFilters(entry, config.snapshot.filters.storyIds),
    );
