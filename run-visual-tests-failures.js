...
function readResultsJSON(jsonFilePath) {
  if (!existsSync(jsonFilePath)) {
    ...
    console.error('💡 Run the visual tests first to generate results: npm run test:visual-tests');
    return null;
  }
...
async function main() {
  ...
  const defaultJsonPath = path.join(__dirname, '../logs/visual-test-results.json');
  const jsonFilePath = args.length > 0 ? path.resolve(args[0]) : defaultJsonPath;
